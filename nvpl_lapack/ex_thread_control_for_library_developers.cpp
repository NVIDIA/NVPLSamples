...
void lib_call_with_internal_nvpl_lapack_usage(
        int nthreads, int n_, float alpha, float *x, int incx_, float tau) {
    nvpl_int_t n = n_;
    nvpl_int_t incx = incx_;

    printf("Calling lib_call_with_internal_nvpl_lapack_usage() with "
           "nthreads = %d\n",
            nthreads);
...
void lib_call_with_internal_nvpl_lapack_usage_bad(
        int nthreads, int n_, float alpha, float *x, int incx_, float tau) {
    nvpl_int_t n = n_;
    nvpl_int_t incx = incx_;

    printf("Calling lib_call_with_internal_nvpl_lapack_usage_bad() "
           "with nthreads = %d\n",
            nthreads);

    /* Just setting the local number of threads for NVPL LAPACK to use.
...
int main() {
    ...
    lib_call_with_internal_nvpl_lapack_usage(
            library_nthreads, n, alpha, x0, incx, tau);

    printf("(after calling lib_call_with_internal_nvpl_lapack_usage() LAPACK "
           "setting for nthreads = %d \n",
            nvpl_lapack_get_max_threads());

    lib_call_with_internal_nvpl_lapack_usage_bad(
            library_nthreads, n, alpha, x1, incx, tau);

    printf("(after calling lib_call_with_internal_nvpl_lapack_usage_bad() "
