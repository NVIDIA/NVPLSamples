...
int main() {
    ...
    //     4   2   5
    //     5   4   3
    double A[lda * m] = {1, 1, 1, 2, 3, 4, 3, 5, 2, 4, 2, 5, 5, 4, 3};
    ...
    //      16  16
    //      18  16
    double b[ldb * m] = {-10, -3, 12, 14, 14, 12, 16, 16, 18, 16};

    printf("NVPL LAPACK version: %d\n", nvpl_lapack_get_version());

    // Print inputs.
    print_dmatrix_rowmajor("Entry Matrix A", m, n, A, lda);
    print_dmatrix_rowmajor("Right Hand Side b", m, nrhs, b, ldb);
    printf("\n");
    printf("LAPACKE_dgels (row-major, high-level) Example Program Results\n");

    // Solve least square problem: min_x || A * x - b ||.
    char no_transpose = 'N';
    nvpl_int_t info = LAPACKE_dgels(
    ...
    // Print solution.
    print_dmatrix_rowmajor("Solution", n, nrhs, b, ldb);
    printf("\n");
