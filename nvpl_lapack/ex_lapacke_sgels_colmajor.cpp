...
int main()
{
    ...
    //     4   2   5
    //     5   4   3
    float A[lda * n] = {1, 2, 3, 4, 5, 1, 3, 5, 2, 4, 1, 4, 2, 5, 3};
    ...
    //      16  16
    //      18  16
    float b[ldb * nrhs] = {-10, 12, 14, 16, 18, -3, 14, 12, 16, 16};

    printf("NVPL LAPACK version: %d\n", nvpl_lapack_get_version());

    // Print inputs.
    print_smatrix_colmajor("Entry Matrix A", m, n, A, lda);
    print_smatrix_colmajor("Right Hand Side b", m, nrhs, b, ldb);
    printf("\n");
    printf("LAPACKE_sgels (col-major, high-level) Example Program Results\n");

    // Solve least square problem: min_x || A * x - b ||.
    char no_transpose = 'N';
    nvpl_int_t info = LAPACKE_sgels(LAPACK_COL_MAJOR, no_transpose, m, n, nrhs,
            A, lda, b, ldb);
    ...
    // Print solution.
    print_smatrix_colmajor("Solution", n, nrhs, b, ldb);
    printf("\n");
