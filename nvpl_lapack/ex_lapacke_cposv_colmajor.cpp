...
int main()
{
    ...
    //     -3    0   15   -8
    //      7   17   -8   23
    nvpl_scomplex_t A[lda * n] = {4, 5, -3, 7, 5, 16, 0, 17, -3, 0, 15, -8, 7, 17, -8, 23};
    ...
    printf("\n");
    printf("LAPACKE_cposv (col-major, high-level) Example Program Results\n");

    // Solve A * x = b using Cholesky decomposition.
    char lower = 'L';
    nvpl_int_t info = LAPACKE_cposv(LAPACK_COL_MAJOR, lower, n, nrhs, A, lda,
            b, ldb);
    ...
    print_cmatrix_colmajor("Cholesky factor", n, n, A, lda);
    printf("\n");
