...
int main() {
    ...
    nvpl_scomplex_t work_query;
    float *rwork = NULL;
    nvpl_int_t lwork = -1;
    nvpl_int_t lrwork = 3 * n - 2;
    nvpl_int_t info;

    // Allocate real workspace.
    rwork = (float *)malloc(sizeof(float) * lrwork);
    if (!rwork) {
    ...
    // Workspace query.
    NVPL_LAPACK_cheev(
            &jobz, &uplo, &n, A, &lda, w, &work_query, &lwork, rwork, &info);
    lwork = (nvpl_int_t)nvpl_scomplex_t_real(work_query);
    ...
    NVPL_LAPACK_cheev(&jobz, &uplo, &n, A, &lda, w, work, &lwork, rwork, &info);
    ...
    // Print eigenvectors.
    print_cmatrix_colmajor("Eigenvectors", n, n, A, lda);
    printf("\n");
