#include <stdio.h>
#include <stdlib.h>
#include <nvpl_lapack.h>

#include "utils.h"

int main() {
    // Initialization.
    const nvpl_int_t n = 4;   // Number of rows and columns of A.
    const nvpl_int_t lda = n; // Leading dimension of A.

    // Matrix A has n x n dimensions with lda = n for column-major ordering.
    // A = 13    3   -6   -5
    //      3   13   -6   -5
    //     -6   -6   31    7
    //     -5   -5    7   23
    nvpl_dcomplex_t A[lda * n]
            = {13, 3, -6, -5, 3, 13, -6, -5, -6, -6, 31, 7, -5, -5, 7, 23};

    // Eigenvalues of A.
    double w[n] = {0};

    printf("NVPL LAPACK version: %d\n", nvpl_lapack_get_version());

    // Print inputs.
    print_zmatrix_colmajor("Entry Matrix A", n, n, A, lda);
    printf("\n");
    printf("zheev Example Program Results\n");

    // Compute eigenvalue decomposition.
    char jobz = 'V';
    char uplo = 'L';
    nvpl_dcomplex_t *work = NULL;
    nvpl_dcomplex_t work_query;
    double *rwork = NULL;
    nvpl_int_t lwork = -1;
    nvpl_int_t lrwork = 3 * n - 2;
    nvpl_int_t info;

    // Allocate real workspace.
    rwork = (double *)malloc(sizeof(double) * lrwork);
    if (!rwork) {
        printf("Not enough memory for work arrays.\n");
        exit(-1);
    }

    // Workspace query.
    NVPL_LAPACK_zheev(
            &jobz, &uplo, &n, A, &lda, w, &work_query, &lwork, rwork, &info);
    lwork = (nvpl_int_t)nvpl_dcomplex_t_real(work_query);
    work = (nvpl_dcomplex_t *)malloc(sizeof(nvpl_dcomplex_t) * lwork);
    if (!work) {
        printf("Not enough memory for work arrays.\n");
        free(rwork);
        exit(-1);
    }

    // Compute eigenvalues and eigenvectors.
    NVPL_LAPACK_zheev(&jobz, &uplo, &n, A, &lda, w, work, &lwork, rwork, &info);

    // Any errors?
    if (info < 0)
        printf("Illegal input argument.\n");
    else if (info > 0)
        printf("The algorithm failed to converge.\n");

    if (info != 0) {
        free(work);
        free(rwork);
        return info;
    }

    // Print eigenvalues.
    print_dvector("Eigenvalues", n, w);
    printf("\n");

    // Print eigenvectors.
    print_zmatrix_colmajor("Eigenvectors", n, n, A, lda);
    printf("\n");

    free(work);
    free(rwork);
    return info;
}