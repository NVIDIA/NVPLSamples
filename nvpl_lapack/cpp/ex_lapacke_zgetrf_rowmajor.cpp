...
int main() {
    ...
    // Print inputs.
    print_zmatrix_rowmajor("Entry Matrix A (row-major layout)", m, n, A, lda);
    printf("\n");
    printf("LAPACKE_zgetrf (row-major) Example Program Results\n");

    // Compute LU factorization P * A = L * U.
    nvpl_int_t info = LAPACKE_zgetrf(LAPACK_ROW_MAJOR, m, n, A, lda, ipiv);
    ...
    // Print LU factors.
    print_zmatrix_rowmajor("LU factors", m, n, A, lda);
    printf("\n");
