...
int main() {
    ...
    //     -1   2   0   1
    //      3  -1   2   2
    float A[lda * m] = {1, 2, 2, -3, 0, 1, 2, 1, -1, 2, 0, 1, 3, -1, 2, 2};
    ...
    // Print inputs.
    print_smatrix_rowmajor("Entry Matrix A (row-major layout)", m, n, A, lda);
    printf("\n");
    printf("LAPACKE_sgetrf (row-major) Example Program Results\n");

    // Compute LU factorization P * A = L * U.
    nvpl_int_t info = LAPACKE_sgetrf(LAPACK_ROW_MAJOR, m, n, A, lda, ipiv);
    ...
    // Print LU factors.
    print_smatrix_rowmajor("LU factors", m, n, A, lda);
    printf("\n");
