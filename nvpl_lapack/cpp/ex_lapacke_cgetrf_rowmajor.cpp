...
int main() {
    ...
    // Print inputs.
    print_cmatrix_rowmajor("Entry Matrix A (row-major layout)", m, n, A, lda);
    printf("\n");
    printf("LAPACKE_cgetrf (row-major) Example Program Results\n");

    // Compute LU factorization P * A = L * U.
    nvpl_int_t info = LAPACKE_cgetrf(LAPACK_ROW_MAJOR, m, n, A, lda, ipiv);
    ...
    // Print LU factors.
    print_cmatrix_rowmajor("LU factors", m, n, A, lda);
    printf("\n");
