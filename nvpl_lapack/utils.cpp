...
template <>
struct is_element_complex<nvpl_scomplex_t> : std::true_type {};
template <>
struct is_element_complex<nvpl_dcomplex_t> : std::true_type {};
...
template <>
void print_element<nvpl_scomplex_t>(
        const nvpl_scomplex_t *mat, nvpl_int_t index) {
    printf(" %6.2f + %6.2fj", nvpl_scomplex_t_real(mat[index]),
            nvpl_scomplex_t_imag(mat[index]));
}

template <>
void print_element<nvpl_dcomplex_t>(
        const nvpl_dcomplex_t *mat, nvpl_int_t index) {
    printf(" %6.2f + %6.2fj", nvpl_dcomplex_t_real(mat[index]),
            nvpl_dcomplex_t_imag(mat[index]));
}
...
template <typename T>
void print_matrix_rowmajor_impl(const char *desc, nvpl_int_t m, nvpl_int_t n,
        const T *flat_mat, nvpl_int_t ldn) {
    nvpl_int_t i, j;
    printf("\n %s\n", desc);

    for (i = 0; i < m; i++) {
        for (j = 0; j < n; j++)
            print_element(flat_mat, i * ldn + j);
        printf("\n");
    }
}
...
/* Auxiliary routine: printing a matrix */
template <typename T>
void print_matrix_colmajor_impl(const char *desc, nvpl_int_t m, nvpl_int_t n,
        const T *flat_mat, nvpl_int_t ldm) {
    nvpl_int_t i, j;
    printf("\n %s\n", desc);

    for (i = 0; i < m; i++) {
        for (j = 0; j < n; j++)
            print_element(flat_mat, i + j * ldm);
        printf("\n");
    }
}
