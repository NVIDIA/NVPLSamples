...
constexpr std::array<int, 5> arbitrary_step = {2, 4, 1, 2, 3};
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        std::cout<<"Usage: ./r2c_c2r_double_many_example NX [NY [NZ]]\n"
                 <<"Arguments:\n"
    ...
    const int howmany = set_howmany<double>(howmany_default, std::max(std::max(r2c_idist, r2c_odist), std::max(c2r_idist, c2r_odist)));
    if (howmany <= 0) {
