...
logical function run_align_api_test() result(success)
    ...
    complex(C_DOUBLE_COMPLEX), pointer :: complex_ary(:)
    type(C_PTR) :: complex_ptr

    real(C_DOUBLE), pointer :: real_ary(:)
    type(C_PTR) :: real_ptr
    ...
    complex_ptr = fftw_alloc_complex(int(nx, C_SIZE_T))
    ...
    if (fftw_alignment_of(real_ary) .ne. int(0, C_INT)) then
        success = .false.
    end if

    call fftw_free(complex_ptr)

    real_ptr = fftw_alloc_real(int(nx, C_SIZE_T))
...
logical function run_1d_api_test() result(success)
    ...
    complex(C_DOUBLE_COMPLEX), pointer :: c2c(:)
    complex(C_DOUBLE_COMPLEX), pointer :: c2c_tmp(:)
    ...
    real(C_DOUBLE), pointer :: r2c(:)
    integer(C_INT), dimension(1) :: c2r_count
    complex(C_DOUBLE_COMPLEX), pointer :: c2r(:)
    ...
    c2c_ptr = fftw_alloc_complex(int(product(c2c_count), C_SIZE_T))
    call c_f_pointer(c2c_ptr, c2c, [product(c2c_count)]);
    c2c_tmp_ptr = fftw_alloc_complex(int(product(c2c_count), C_SIZE_T))
    call c_f_pointer(c2c_tmp_ptr, c2c_tmp, [product(c2c_count)]);
    allocate(c2c_c2c_solution(product(c2c_count)))

    r2c_ptr = fftw_alloc_real(int(product(r2c_count), C_SIZE_T))
    call c_f_pointer(r2c_ptr, r2c, [product(r2c_count)]);
    c2r_ptr = fftw_alloc_complex(int(product(c2r_count), C_SIZE_T))
    ...
    plan_c2c_forward = fftw_plan_dft_1d(c2c_count(1), c2c, c2c_tmp, FFTW_FORWARD, FFTW_MEASURE)
    plan_c2c_backward = fftw_plan_dft_1d(c2c_count(1), c2c_tmp, c2c, FFTW_BACKWARD, FFTW_MEASURE)

    call fftw_execute_dft(plan_c2c_forward, c2c, c2c_tmp)
    call fftw_execute_dft(plan_c2c_backward, c2c_tmp, c2c)

    call normalize_complex(c2c, product(c2c_count))

    call fftw_destroy_plan(plan_c2c_forward)
    call fftw_destroy_plan(plan_c2c_backward)

    plan_r2c = fftw_plan_dft_r2c_1d(r2c_count(1), r2c, c2r, FFTW_MEASURE)
    plan_c2r = fftw_plan_dft_c2r_1d(r2c_count(1), c2r, r2c, FFTW_MEASURE)

    call fftw_execute_dft_r2c(plan_r2c, r2c, c2r)
    call fftw_execute_dft_c2r(plan_c2r, c2r, r2c)
    ...
    if (success .eqv. .true.) then
        success = validate_complex(c2c, c2c_c2c_solution, product(c2c_count), 1.0d-5)
    end if

    if (success .eqv. .true.) then
        success = validate_real(r2c, r2c_c2r_solution, product(r2c_count), 1.0d-5)
    end if

    deallocate(r2c_c2r_solution)
    call fftw_free(c2r_ptr)
    call fftw_free(r2c_ptr)

    deallocate(c2c_c2c_solution)
    call fftw_free(c2c_tmp_ptr)
    call fftw_free(c2c_ptr)
...
logical function run_2d_api_test() result(success)
    ...
    plan_c2c_forward = fftw_plan_dft_2d(c2c_count(1), c2c_count(2), c2c, c2c_tmp, FFTW_FORWARD, FFTW_MEASURE)
    plan_c2c_backward = fftw_plan_dft_2d(c2c_count(1), c2c_count(2), c2c_tmp, c2c, FFTW_BACKWARD, FFTW_MEASURE)
    ...
    plan_r2c = fftw_plan_dft_r2c_2d(r2c_count(1), r2c_count(2), r2c, c2r, FFTW_MEASURE)
    plan_c2r = fftw_plan_dft_c2r_2d(r2c_count(1), r2c_count(2), c2r, r2c, FFTW_MEASURE)
...
logical function run_3d_api_test() result(success)
    ...
    plan_c2c_forward = fftw_plan_dft_3d(c2c_count(1), c2c_count(2), c2c_count(3), c2c, c2c_tmp, FFTW_FORWARD, FFTW_MEASURE)
    plan_c2c_backward = fftw_plan_dft_3d(c2c_count(1), c2c_count(2), c2c_count(3), c2c_tmp, c2c, FFTW_BACKWARD, FFTW_MEASURE)
    ...
    plan_r2c = fftw_plan_dft_r2c_3d(r2c_count(1), r2c_count(2), r2c_count(3), r2c, c2r, FFTW_MEASURE)
    plan_c2r = fftw_plan_dft_c2r_3d(r2c_count(1), r2c_count(2), r2c_count(3), c2r, r2c, FFTW_MEASURE)
...
logical function run_many_api_test() result(success)
    ...
    c2c_ptr = fftw_alloc_complex(int(howmany * product(c2c_count), C_SIZE_T))
    call c_f_pointer(c2c_ptr, c2c, [howmany * product(c2c_count)]);
    c2c_tmp_ptr = fftw_alloc_complex(int(howmany * product(c2c_count), C_SIZE_T))
    call c_f_pointer(c2c_tmp_ptr, c2c_tmp, [howmany * product(c2c_count)]);
    allocate(c2c_c2c_solution(howmany * product(c2c_count)))

    r2c_ptr = fftw_alloc_real(int(howmany * product(r2c_count), C_SIZE_T))
    call c_f_pointer(r2c_ptr, r2c, [howmany * product(r2c_count)]);
    c2r_ptr = fftw_alloc_complex(int(howmany * product(c2r_count), C_SIZE_T))
    ...
    plan_c2c_forward = fftw_plan_many_dft(size(c2c_count), c2c_count, howmany, &
                                          c2c,     c2c_embed,     c2c_stride,     c2c_dist, &
                                          c2c_tmp, c2c_tmp_embed, c2c_tmp_stride, c2c_tmp_dist, &
                                          FFTW_FORWARD, FFTW_MEASURE)
    plan_c2c_backward = fftw_plan_many_dft(size(c2c_count), c2c_count, howmany, &
                                           c2c_tmp, c2c_tmp_embed, c2c_tmp_stride, c2c_tmp_dist, &
                                           c2c,     c2c_embed,     c2c_stride,     c2c_dist, &
    ...
    plan_r2c = fftw_plan_many_dft_r2c(size(r2c_count), r2c_count, howmany, &
                                      r2c, r2c_embed, r2c_stride, r2c_dist, &
                                      c2r, c2r_embed, c2r_stride, c2r_dist, &
                                      FFTW_MEASURE)
    plan_c2r = fftw_plan_many_dft_c2r(size(r2c_count), r2c_count, howmany, &
                                      c2r, c2r_embed, c2r_stride, c2r_dist, &
                                      r2c, r2c_embed, r2c_stride, r2c_dist, &
                                      FFTW_MEASURE)
...
logical function run_thread_api_test() result(success)
    ...
    success = (fftw_init_threads() /= 0);

    call fftw_plan_with_nthreads(nthread);

    success = (fftw_planner_nthreads() == nthread);
    plan_c2c_forward = fftw_plan_dft_1d(c2c_count(1), c2c, c2c_tmp, FFTW_FORWARD, FFTW_MEASURE)
