...
int main() {
    ...
    printf("\nExample: cblas_sgemm_batch_strided for the matrix-matrix multiplication of a batch of matrices\n\n");
    ...
    // allocate memory
    A = (float *)malloc((strideA * (int64_t)batch_size) * sizeof(float));
    B = (float *)malloc((strideB * (int64_t)batch_size) * sizeof(float));
    C = (float *)malloc((strideC * (int64_t)batch_size) * sizeof(float));

    // fill data
    for (nvpl_int_t i = 0; i < batch_size; ++i) {
        fill_smatrix(A + strideA * i, rowsA, colsA, lda, order, Full, CblasNonUnit);
        fill_smatrix(B + strideB * i, rowsB, colsB, ldb, order, Full, CblasNonUnit);
        fill_smatrix(C + strideC * i, rowsC, colsC, ldc, order, Full, CblasNonUnit);
    }

    // print input data
    for (nvpl_int_t i = 0; i < batch_size; ++i) {
        print_smatrix(A + strideA * i, rowsA, colsA, lda, "A", order);
        print_smatrix(B + strideB * i, rowsB, colsB, ldb, "B", order);
        print_smatrix(C + strideC * i, rowsC, colsC, ldc, "C", order);
    }

    // call cblas_sgemm_batch_strided
    cblas_sgemm_batch_strided(order, transA, transB, M, N, K, alpha, A, lda, strideA,
