...
int main() {
    ...
    // allocate memory
    A = (nvpl_scomplex_t *)malloc((strideA * (int64_t)batch_size) * sizeof(nvpl_scomplex_t));
    B = (nvpl_scomplex_t *)malloc((strideB * (int64_t)batch_size) * sizeof(nvpl_scomplex_t));
    C = (nvpl_scomplex_t *)malloc((strideC * (int64_t)batch_size) * sizeof(nvpl_scomplex_t));

    // fill data
    for (nvpl_int_t i = 0; i < batch_size; ++i) {
        fill_cmatrix(A + strideA * i, rowsA, colsA, lda, order, Full, CblasNonUnit);
        fill_cmatrix(B + strideB * i, rowsB, colsB, ldb, order, Full, CblasNonUnit);
        fill_cmatrix(C + strideC * i, rowsC, colsC, ldc, order, Full, CblasNonUnit);
    }

    // print input data
    for (nvpl_int_t i = 0; i < batch_size; ++i) {
        print_cmatrix(A + strideA * i, rowsA, colsA, lda, "A", order);
        print_cmatrix(B + strideB * i, rowsB, colsB, ldb, "B", order);
        print_cmatrix(C + strideC * i, rowsC, colsC, ldc, "C", order);
    }

    // call cblas_cgemm_batch_strided
    cblas_cgemm_batch_strided(order, transA, transB, M, N, K, &alpha, A, lda, strideA,
