...
int main() {
    ...
    nvpl_scomplex_t alpha[GROUP_COUNT] = {{1.0f, 2.0f}, {1.0f, 2.0f}};
    nvpl_scomplex_t beta[GROUP_COUNT] = {{1.0f, 2.0f}, {1.0f, 2.0f}};
    ...
    nvpl_scomplex_t **A_array = NULL;
    nvpl_scomplex_t **B_array = NULL;
    nvpl_scomplex_t **C_array = NULL;
    nvpl_int_t total_batch_count = 0;


    printf("\nExample: cblas_cgemm_batch for the matrix-matrix multiplication of a batch of matrices\n\n");

    for (nvpl_int_t i = 0; i < GROUP_COUNT; ++i) {
        total_batch_count += group_size[i];
    }

    A_array = (nvpl_scomplex_t **)malloc(total_batch_count * sizeof(nvpl_scomplex_t *));
    B_array = (nvpl_scomplex_t **)malloc(total_batch_count * sizeof(nvpl_scomplex_t *));
    C_array = (nvpl_scomplex_t **)malloc(total_batch_count * sizeof(nvpl_scomplex_t *));
    ...
    for (nvpl_int_t i = 0; i < GROUP_COUNT; ++i) {
        ...
        for (nvpl_int_t j = 0; j < group_size[i]; ++j) {
            // allocate memory
            A = (nvpl_scomplex_t *)malloc(lda[i] * colsA * sizeof(nvpl_scomplex_t));
            B = (nvpl_scomplex_t *)malloc(ldb[i] * colsB * sizeof(nvpl_scomplex_t));
            C = (nvpl_scomplex_t *)malloc(ldc[i] * colsC * sizeof(nvpl_scomplex_t));

            // fill data
            fill_cmatrix(A, rowsA, colsA, lda[i], order, Full, CblasNonUnit);
            fill_cmatrix(B, rowsB, colsB, ldb[i], order, Full, CblasNonUnit);
            fill_cmatrix(C, rowsC, colsC, ldc[i], order, Full, CblasNonUnit);

            // print input data
            print_cmatrix(A, rowsA, colsA, lda[i], "A", order);
            print_cmatrix(B, rowsB, colsB, ldb[i], "B", order);
            print_cmatrix(C, rowsC, colsC, ldc[i], "C", order);
    ...
    // call cblas_cgemm_batch
    cblas_cgemm_batch(order, transA, transB, M, N, K, alpha, (const void  **)A_array, lda, (const void  **)B_array,
            ldb, beta, (void **)C_array, ldc, GROUP_COUNT, group_size);

    // print result
    printf("\nAfter cblas_cgemm_batch operation:\n");
    idx = 0;
    for (nvpl_int_t i = 0; i < GROUP_COUNT; ++i) {
        printf("Group=%" PRId64 "\n", (int64_t)i);
        for (nvpl_int_t j = 0; j < group_size[i]; ++j) {
            printf("Matrix idx=%" PRId64 "\n", (int64_t)idx);
            print_cmatrix(C_array[idx], M[i], N[i], ldc[i], "C", order);
            free(A_array[idx]);
            free(B_array[idx]);
            free(C_array[idx]);
            idx++;
