...
!
!******************************************************************************
      program   CGEMM_BATCH_STRIDED_MAIN
*
      implicit none
     ...
     &                  lda=2, ldb=2, ldc=2)
      ...
      complex          a(lda,k,batch_size), b(ldb,n,batch_size),
     &                 c(ldc,n,batch_size)
...
*       Print input data
      ...
      do i = 1, batch_size
          call fill_cmatrix(a(:,:,i),m,k,lda,'F','N')
          call fill_cmatrix(b(:,:,i),k,n,ldb,'F','N')
          call fill_cmatrix(c(:,:,i),m,n,ldc,'F','N')
          call print_cmatrix(a(:,:,i),m,k,lda,'A')
          call print_cmatrix(b(:,:,i),k,n,ldb,'B')
          call print_cmatrix(c(:,:,i),m,n,ldc,'C')
      end do
*
*      Call CGEMM_BATCH_STRIDED subroutine
      call CGEMM_BATCH_STRIDED(transa,transb,m,n,k,alpha,a,lda,
     &     stridea,b,ldb,strideb,beta,c,ldc,stridec,batch_size)
*
      print*
      print 102, 'After CGEMM_BATCH_STRIDED operation:'
      do i = 1, batch_size
          call print_cmatrix(c(:,:,i),m,n,ldc,'C')
      end do

      stop
 99   format('Example: CGEMM_BATCH_STRIDED for the matrix-matrix',
