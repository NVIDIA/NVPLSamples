...
*
     ...
     &                 (ldc(i), i=1,grp_cnt)/grp_cnt*2/
      double precision alpha(grp_cnt), beta(grp_cnt)
     ...
     &                 (beta(i), i=1,grp_cnt)/grp_cnt*2.0/
      ...
      double precision a(rmaxa,cmaxa,maxmat), b(rmaxb,cmaxb,maxmat),
...
*
     ...
     &        transa(ig), transb(ig)
          ...
          do is = 1, grp_size(ig)
              ...
              call fill_dmatrix(a(:,:,matidx),m(ig),k(ig),
     &                          lda(ig),'F','N')
              call fill_dmatrix(b(:,:,matidx),k(ig),n(ig),
     &                          ldb(ig),'F','N')
              call fill_dmatrix(c(:,:,matidx),m(ig),n(ig),
     &                          ldc(ig),'F','N')
              call print_dmatrix(a(:,:,matidx),m(ig),k(ig),lda(ig),'A')
              call print_dmatrix(b(:,:,matidx),k(ig),n(ig),ldb(ig),'B')
              call print_dmatrix(c(:,:,matidx),m(ig),n(ig),ldc(ig),'C')
              matidx = matidx + 1
          end do
      end do
*
*      Call DGEMM_BATCH subroutine
      call DGEMM_BATCH(transa,transb,m,n,k,alpha,a_array,lda,b_array,
     &                 ldb,beta,c_array,ldc,grp_cnt,grp_size)
*
      print*
      print 102, 'After DGEMM_BATCH operation:'
      print*
      matidx = 1
      do ig = 1, grp_cnt
          print 104, ig
      ...
      stop
 99   format('Example: DGEMM_BATCH for the matrix-matrix multiplication'
