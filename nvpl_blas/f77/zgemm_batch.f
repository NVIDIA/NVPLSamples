...
*
     ...
     &                 (ldc(i), i=1,grp_cnt)/grp_cnt*2/
      double complex   alpha(grp_cnt), beta(grp_cnt)
...
*
     ...
     &        transa(ig), transb(ig)
          ...
          do is = 1, grp_size(ig)
              ...
              call fill_zmatrix(a(:,:,matidx),m(ig),k(ig),
     &                          lda(ig),'F','N')
              call fill_zmatrix(b(:,:,matidx),k(ig),n(ig),
     &                          ldb(ig),'F','N')
              call fill_zmatrix(c(:,:,matidx),m(ig),n(ig),
     &                          ldc(ig),'F','N')
              call print_zmatrix(a(:,:,matidx),m(ig),k(ig),lda(ig),'A')
              call print_zmatrix(b(:,:,matidx),k(ig),n(ig),ldb(ig),'B')
              call print_zmatrix(c(:,:,matidx),m(ig),n(ig),ldc(ig),'C')
              matidx = matidx + 1
          end do
      end do
*
*      Call ZGEMM_BATCH subroutine
      call ZGEMM_BATCH(transa,transb,m,n,k,alpha,a_array,lda,b_array,
     &                 ldb,beta,c_array,ldc,grp_cnt,grp_size)
*
      print*
      print 102, 'After ZGEMM_BATCH operation:'
      print*
      matidx = 1
      do ig = 1, grp_cnt
          print 104, ig
